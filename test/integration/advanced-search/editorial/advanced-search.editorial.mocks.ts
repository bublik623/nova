import { Page } from "@playwright/test";
import { mockRequest } from "../../utils/mockRequest";
import {
  editorialSearchHitsWithDefaultLanguageFilterAndNoQuery as editorialSearchHitsWithDefaultLanguageFilterAndNoQuery,
  editorialSearchHitsWithQuery,
  createNthEditorialPage,
  editorialResultsWithFilters,
  reportFileName,
  reportFileContent,
} from "./advanced-search.editorial.data";
import { mockMasterData } from "../../utils/mockMasterData";

export const contentQueryBaseURL = "http://localhost:8083";
export const masterDataBaseURL = "http://localhost:8095";

export default async function (page: Page) {
  await mockRequest(page, `${contentQueryBaseURL}/distribution-content/report*`, "POST", undefined, () => {
    return {
      headers: { "content-disposition": `form-data; name="attachment"; filename="${reportFileName}"` },
      body: reportFileContent,
    };
  });

  await mockRequest(page, `${contentQueryBaseURL}/distribution-content*`, "GET", undefined, (request) => {
    const urlParts = request.url().split("?");
    const querystring = toMap(urlParts[1]);
    const hasQuery = querystring.get("query")?.length;
    const hasFilters = querystring.get("filters")?.length;
    const hasOnlyDefaultLanguageFilter =
      hasFilters &&
      querystring.get("filters") === "experience_content.language_code%3Din%3D(de,dk,en,es,fi,fr,it,nl,no,pl,pt,ru,se)";

    let body;

    if (hasQuery) {
      body = editorialSearchHitsWithQuery;
    } else if (hasFilters && !hasOnlyDefaultLanguageFilter) {
      body = editorialResultsWithFilters;
    } else {
      body = editorialSearchHitsWithDefaultLanguageFilterAndNoQuery;
    }

    if (querystring.get("offset") !== "0" && querystring.get("limit") !== "0") {
      body = createNthEditorialPage(+querystring.get("offset")! / +querystring.get("limit")! + 1);
    }

    return { body, headers: { "total-counts": "1000" } };
  });

  await mockMasterData(page);
}

function toMap(querystring: string): Map<string, string> {
  const result = new Map();

  querystring.split("&").forEach((param) => {
    const keyValue = param.split("=");
    result.set(keyValue[0], keyValue[1]);
  });

  return result;
}
