...
import { mockOpenAvailabilitiesFixedValidity, mockOfferExperience } from "../availability.data";
...
let mockAvailabilitiesResponse = [...mockOpenAvailabilitiesFixedValidity];
let pageObject: ExperienceAvailabilityPage;
test.beforeEach(async ({ page }) => {
  await mockApi(page);

  // Mock offer service experience
  await page.route(`${offerServiceURL}/experiences/test-experience-open-fixed-validity*`, async (route, req) => {
    switch (req.method()) {
      case "GET":
        await route.fulfill({
          status: 200,
          body: JSON.stringify({
            ...mockOfferExperience(ExperienceType.NO_CALENDAR_FIXED_VALIDITY),
          }),
        });
        break;
      default:
  ...
  await page.goto("/experience/test-experience-open-fixed-validity/options/raw/test-option-id/availability");
  await page.waitForSelector(testId("AvailabilityIndexPage"));

  pageObject = new ExperienceAvailabilityPage(page);
});
test("Availability Page - Open Ticket - Fixed Validity", async () => {
  await test.step("view all availabilities", async () => {
    ...
    expect(await pageObject.findAvailabilityCard(0).findValidityInput().inputValue()).toContain("12");
